...
import PRCelebration, { PREntry } from '@/components/ui/PRCelebration';
...
// Previous-session data: exercise_id → array of {set_number, reps, weight}
type PrevData = Record<string, { set_number: number; reps: number; weight: number }[]>;
...
function LogWorkoutForm() {
  ...
  // Track which exercise IDs we've already fetched prev data for
  const fetchedPrevIds = useRef<Set<string>>(new Set());

  // ── init: load routines + exercises (+ edit mode) ───────────────────
  useEffect(() => {
    async function init() {
      if (!user) return;
      ...
      if (editId) {
        const { data: logData } = await supabase
          .from('workout_logs').select('*').eq('id', editId).single();
        if (logData) {
  ...
  // ── fetch previous-session data for new exercise IDs ────────────────
  const loadPrevData = useCallback(async (exerciseIds: string[]) => {
    if (!user || exerciseIds.length === 0) return;
    const newIds = exerciseIds.filter(id => !fetchedPrevIds.current.has(id));
    if (newIds.length === 0) return;
    newIds.forEach(id => fetchedPrevIds.current.add(id));

    // Last ~30 logs for this user
    ...
    if (recentLogIds.length === 0) return;

    const { data: prevSets } = await supabase
      .from('workout_sets')
      .select('exercise_id, set_number, reps, weight, workout_log_id')
    ...
    setPrevData(prev => {
      const updated = { ...prev };
      for (const exId of newIds) {
        const exSets = prevSets.filter(s => s.exercise_id === exId);
        if (exSets.length === 0) continue;
        // recentLogIds is already sorted most-recent first
        const mostRecentLogId = recentLogIds.find(id =>
          exSets.some(s => s.workout_log_id === id)
        );
        if (!mostRecentLogId) continue;
        updated[exId] = exSets
          .filter(s => s.workout_log_id === mostRecentLogId)
          .sort((a, b) => a.set_number - b.set_number)
          .map(s => ({ set_number: s.set_number, reps: s.reps || 0, weight: Number(s.weight) || 0 }));
      }
      return updated;
    });
  }, [user, editId]);
  ...
  function removeSet(exerciseId: string, setNumber: number) {
    ...
    setDoneSets(prev => { const n = new Set(prev); n.delete(key); return n; });
  }
  ...
  function markSetDone(exerciseId: string, setNumber: number) {
    const key = `${exerciseId}-${setNumber}`;
    setDoneSets(prev => {
  ...
  async function handleSave() {
    ...
    // 1. Detect PRs BEFORE saving (compare against existing history)
    let detectedPRs: PREntry[] = [];
    if (!editId && user && sets.length > 0) {
      try {
        const exerciseIds = [...new Set(sets.map(s => s.exercise_id))];

        // Current session maxes
        const currentMaxes: Record<string, number> = {};
        for (const exId of exerciseIds) {
          const weights = sets.filter(s => s.exercise_id === exId && s.weight > 0).map(s => s.weight);
          if (weights.length > 0) currentMaxes[exId] = Math.max(...weights);
        }

        // Historical bests
        const { data: userLogs } = await supabase
          .from('workout_logs').select('id').eq('user_id', user.id);
        ...
        if (logIds.length > 0) {
          ...
          const historicalBests: Record<string, number> = {};
          histSets?.forEach(s => {
            const w = Number(s.weight) || 0;
            if (w > (historicalBests[s.exercise_id] ?? 0)) historicalBests[s.exercise_id] = w;
          });

          detectedPRs = Object.entries(currentMaxes)
            .filter(([exId, max]) => max > (historicalBests[exId] ?? 0))
            .map(([exId, max]) => ({
              exerciseName: sets.find(s => s.exercise_id === exId)?.exercise_name ?? 'Unknown',
              newWeight: max,
              prevBest: historicalBests[exId] ?? 0,
              unit,
            }));
        }
      } catch (e) {
        console.error('PR detection error', e);
      }
    }
    ...
    <>
      <div style={{ padding: '0 16px 100px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 24, paddingBottom: 16 }}>
          <button
            onClick={() => router.back()}
            style={{ background: '#1E2430', border: '1px solid #252B36', borderRadius: 12, padding: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', color: '#fff' }}
          >
            <ArrowLeft size={18} />
          </button>
          <div>
        ...
        <div className="card" style={{ padding: 16, marginBottom: 16, borderRadius: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <div>
              ...
              <select
                disabled={!!editId}
                style={{ ...inputStyle, appearance: 'none', opacity: editId ? 0.6 : 1 }}
        ...
        {exerciseIds.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {exerciseIds.map(exerciseId => {
              ...
              const prevSets = prevData[exerciseId] ?? [];

              // Summary hint: "Last: best weight × reps"
              const prevBest = prevSets.length > 0
                ? prevSets.reduce((best, s) => s.weight > best.weight ? s : best, prevSets[0])
                : null;
                ...
                <div key={exerciseId} className="card" style={{ padding: 16, borderRadius: 20 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                    <div>
                      ...
                      {prevBest && (
                        <p style={{ fontSize: '0.72rem', color: '#5A6175', margin: '3px 0 0', fontWeight: 500 }}>
                          Last session: {prevBest.weight > 0 ? `${prevBest.weight}${profile?.weight_unit ?? 'kg'} × ` : ''}{prevBest.reps} reps
                        </p>
                      )}
                    </div>
                    <button
                      ...
                      style={{
                        ...
                        background: 'rgba(255,107,53,0.12)', border: 'none', borderRadius: 99,
                        padding: '6px 12px', cursor: 'pointer', color: '#FF6B35', fontWeight: 600, fontSize: '0.75rem',
                      }}
                    >
                      <Plus size={12} /> Set
                    </button>
                  ...
                  {/* Column headers */}
                  <div style={{ display: 'grid', gridTemplateColumns: '28px 1fr 1fr 36px 28px', gap: '4px 8px', alignItems: 'center', marginBottom: 6 }}>
                    <span style={{ fontSize: '0.68rem', color: '#5A6175', fontWeight: 600 }}>#</span>
                    <span style={{ fontSize: '0.68rem', color: '#5A6175', fontWeight: 600, textAlign: 'center' }}>Reps</span>
                    <span style={{ fontSize: '0.68rem', color: '#5A6175', fontWeight: 600, textAlign: 'center' }}>Weight</span>
                    <span style={{ fontSize: '0.68rem', color: '#5A6175', fontWeight: 600, textAlign: 'center' }}>✓</span>
                    <span />
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {exerciseSets.map(set => {
                      ...
                      const isDone = doneSets.has(key);
                      const prev = prevSets.find(p => p.set_number === set.set_number);

                      return (
                        <div
                          key={key}
                          style={{
                            display: 'grid', gridTemplateColumns: '28px 1fr 1fr 36px 28px',
                            gap: '4px 8px', alignItems: 'center',
                            background: isDone ? 'rgba(200,255,0,0.05)' : 'transparent',
                            borderRadius: 10, padding: '2px 0',
                            transition: 'background 0.2s',
                          }}
                        >
                          <span style={{ fontSize: '0.85rem', color: isDone ? '#C8FF00' : '#8A91A8', fontWeight: 600 }}>
                            {set.set_number}
                          </span>
                          <div style={{ position: 'relative' }}>
                            <input
                              type="number"
                              style={{
                                background: '#1E2430', border: `1px solid ${isDone ? 'rgba(200,255,0,0.25)' : '#252B36'}`,
                                borderRadius: 10, padding: '8px 4px', color: '#fff',
                                fontSize: '0.85rem', textAlign: 'center', outline: 'none', width: '100%',
                              }}
                          ...
                          <div style={{ position: 'relative' }}>
                            <input
                              ...
                              style={{
                                background: '#1E2430', border: `1px solid ${isDone ? 'rgba(200,255,0,0.25)' : '#252B36'}`,
                                borderRadius: 10, padding: '8px 4px', color: '#FF6B35',
                                fontSize: '0.85rem', textAlign: 'center', outline: 'none', fontWeight: 600, width: '100%',
                              }}
                          ...
                          {/* Done checkmark */}
                          <button
                            onClick={() => markSetDone(exerciseId, set.set_number)}
                            style={{
                              width: 32, height: 32,
                              borderRadius: '50%',
                              background: isDone ? 'rgba(200,255,0,0.15)' : '#1E2430',
                              border: `1px solid ${isDone ? '#C8FF00' : '#252B36'}`,
                              cursor: 'pointer',
                              display: 'flex', alignItems: 'center', justifyContent: 'center',
                              transition: 'all 0.18s',
                            }}
                          >
                            <Check size={14} color={isDone ? '#C8FF00' : '#5A6175'} />
                          </button>
